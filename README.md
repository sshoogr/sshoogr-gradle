...
The following list gives an overview of the available configuration options:
 ...
 - `verbose` (defaults to false) - If set to true, plugin produces more debug output.
 
...
"Executing commands" section) options. It has the following properties:
 ...
 - `maxWait` (defaults to 0) - Number of milliseconds to wait for command to finish. If it is set to 0, then plugin will wait forever.
...
section) options. It has the following properties:
 ...
 - `showProgress` (defaults to false) - If set to true, plugin shows additional information regarding file upload/download progress.
 - `verbose` (defaults to false) - If set to true, plugin produces more debug output.
